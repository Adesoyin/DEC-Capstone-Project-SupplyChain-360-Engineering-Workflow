...
def get_postgres_conn():
    ...
    try:
        ...
        logger.info(f"Connecting to DB: {os.getenv('DB_HOST')}:{os.getenv('DB_PORT')}")
...
#         for i, chunk in enumerate(chunks):
#             # Fix UUID issue error I was getting
#             if 'transaction_id' in chunk.columns:
#                 chunk['transaction_id'] = chunk['transaction_id'].astype(str)
...
#     print("=" *20 )
#     logger.info(f"Starting sales transaction ingestion for Business Date: {target_day}")
